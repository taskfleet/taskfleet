...
CREATE TABLE instances (
    ...
    */
    ...
    CHECK (provider_id != ''),
    CHECK (zone != ''),
    CHECK (cpu_count_requested >= 1),
    CHECK (memory_mib_requested >= 1024),
    CHECK (memory_mib_reserved >= 1024),
    CHECK (gpu_count_requested >= 0),
    CHECK (boot_image != ''),
    CHECK (hostname IS NULL OR hostname != ''),
    CHECK (booted_at IS NULL OR booted_at > created_at),
    CHECK (started_at IS NULL OR started_at > booted_at),
    CHECK (is_deletion_triaged = FALSE OR deleted_at IS NOT NULL),
    FOREIGN KEY (provider, instance_type) REFERENCES instance_types (provider, name)
);

CREATE INDEX idx_instances_deleted_at ON instances (deleted_at);
CREATE INDEX idx_instances_is_deletion_triaged ON instances (is_deletion_triaged);
